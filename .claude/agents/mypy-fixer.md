...
Fixes mypy type errors and eliminates `Any` usage in the pyfordpass project.
...
1. Fix all mypy errors reported by `uv run mypy fordpass`
...
Place new TypedDict classes in `fordpass/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`fordpass/typing.py` when shared.
...
1. Run `uv run mypy fordpass` and capture all errors.
...
4. Run `uv run mypy fordpass` again to verify fixes.
