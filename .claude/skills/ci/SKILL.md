...
If any changed files are under `fordpass/` or `tests/`, run the following
...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `fordpass/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `fordpass/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `fordpass/commands/main.py`,
...
For Python files, strip the `fordpass/` prefix and replace `/` with `.` (like module imports).

- Python file `fordpass/media.py` → `media:`.
- Multiple files under `fordpass/commands/` → `commands:`.
- Single command file `fordpass/commands/admin.py` → `commands.admin:`.
